    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 24 }}>
      <div className="glass-panel" style={{ padding: 32 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 32, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#86868b", textTransform: "uppercase", marginBottom: 8 }}>Primary Gaps Detected</div>
            ...
            <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#86868b", textTransform: "uppercase", marginBottom: 8 }}>Strong Matches</div>
      ...
      <div className="glass-panel" style={{ padding: 32 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {evolutionOptions.map((up) => {
            ...
            return (
              <label key={up.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: 14, background: isChecked ? "rgba(124, 58, 237, 0.05)" : "rgba(0, 0, 0, 0.02)", border: isChecked ? "1px solid rgba(124, 58, 237, 0.2)" : "1px solid rgba(0,0,0,0.05)", borderRadius: 12, cursor: "pointer", transition: "all 0.2s" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: isChecked ? "#7c3aed" : "#1d1d1f" }}>{up.label}</div>
                  <div style={{ fontSize: 11, color: "#86868b", marginTop: 2 }}>{up.desc}</div>
                </div>
              </label>
            );
          })}
      ...
      <div className="glass-panel" style={{ padding: 32 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {result?.roadmap?.length > 0 ? result.roadmap.map((step, idx) => (
            <div key={idx} style={{ padding: 16, background: "rgba(0, 0, 0, 0.02)", backdropFilter: "blur(24px) saturate(180%)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#111", marginBottom: 4 }}>{step.phase}</div>
              <div style={{ fontSize: 12, color: "#86868b", marginBottom: 12 }}>{step.focus}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
