    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 24 }}>
      ...
      <div className="glass-panel" style={{ padding: 32 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: "rgba(0, 0, 0, 0.02)", backdropFilter: "blur(24px) saturate(180%)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 16, padding: 18 }}>
            <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#86868b", marginBottom: 4, textTransform: "uppercase" }}>Benchmark Tier</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: "#7c3aed", letterSpacing: "-0.01em", lineHeight: 1.2 }}>{comparison}</div>
            <div style={{ fontSize: 12, color: "#86868b", marginTop: 8, fontStyle: "italic" }}>
          ...
          {result.benchmark_metrics?.explanations && result.benchmark_metrics.explanations.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {result.benchmark_metrics.explanations.map((exp, idx) => (
                      <div key={idx} style={{ fontSize: 12, color: "#86868b", display: "flex", gap: 8, alignItems: "flex-start" }}>
