  ...
  return (
    <div className="glass-panel" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        <div style={{ background: "rgba(0, 0, 0, 0.02)", border: "1px solid rgba(255,255,255,0.15)", padding: 12, borderRadius: 12, textAlign: "center" }}>
          <div style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: "#86868b", textTransform: "uppercase" }}>Pipeline Latency</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#7c3aed", fontFamily: "'DM Mono', monospace", marginTop: 4 }}>{latency}ms</div>
        </div>

        <div style={{ background: "rgba(0, 0, 0, 0.02)", border: "1px solid rgba(255,255,255,0.15)", padding: 12, borderRadius: 12, textAlign: "center" }}>
          <div style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: "#86868b", textTransform: "uppercase" }}>Model Drift</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#059669", fontFamily: "'DM Mono', monospace", marginTop: 4 }}>{drift.toFixed(3)}</div>
        </div>

        <div style={{ background: "rgba(0, 0, 0, 0.02)", border: "1px solid rgba(255,255,255,0.15)", padding: 12, borderRadius: 12, textAlign: "center" }}>
          <div style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: "#86868b", textTransform: "uppercase" }}>Vector Density</div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 24 }}>
        ...
        {rolesReadiness.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            {rolesReadiness.map((role, i) => (
              <div key={i} style={{ background: "rgba(0, 0, 0, 0.02)", backdropFilter: "blur(24px) saturate(180%)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 16, padding: 18 }}>
      ...
      <div className="glass-panel" style={{ padding: 32 }}>
        ...
        {steps.length > 0 ? (
            ...
            {steps.map((stepStr, idx) => {
              ...
              return (
                <div key={idx} style={{ display: "flex", gap: 20, marginBottom: idx === steps.length - 1 ? 0 : 28 }}>
                  <div style={{ width: 12, height: 12, borderRadius: "50%", background: "rgba(0, 0, 0, 0.03)", backdropFilter: "blur(24px) saturate(180%)", border: `2px solid ${stepColor}`, zIndex: 1, marginLeft: -2, marginTop: 18 }} />
                  <div style={{ flex: 1, padding: "16px 20px", background: "rgba(0, 0, 0, 0.02)", backdropFilter: "blur(24px) saturate(180%)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 14 }}>
          ...
          <p style={{ fontSize: 13, color: "#86868b" }}>Trajectory map could not be computed for this profile.</p>
        )}
      </div>
      ...
      <div className="glass-panel" style={{ padding: 32 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {evolutionSteps.map((step, idx) => (
            <div key={idx} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: 14, background: "rgba(0, 0, 0, 0.02)", backdropFilter: "blur(24px) saturate(180%)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#86868b", marginTop: 2 }}>{step.comment}</p>
                </div>
              </div>
