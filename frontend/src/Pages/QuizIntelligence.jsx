...
export default function Quiz() {
  ...
  const handleExportPDF = () => {
    ...
    const opt = {
      margin: 0.5,
      filename: `CareerOS_Report_${track.replace(/\\s+/g, '_')}.pdf`,
  ...
  const handleNext = async () => {
    ...
    if (currentIndex < questions.length - 1) {
      ...
      try {
        ...
        const simData = {
          vectors: {
            "Systems Reasoning": Math.floor(Math.random() * 30) + 70,
            "Architecture Depth": Math.floor(Math.random() * 30) + 65,
            "Production Readiness": Math.floor(Math.random() * 30) + 60,
            "Operational Awareness": Math.floor(Math.random() * 30) + 70
          },
          roadmap: ["Advanced Docker orchestration", "Distributed caching strategies", "Kubernetes cluster security", "CI/CD automated rollbacks"],
          recruiter_signal: "+12% Reliability Signal",
          benchmark: Math.floor(Math.random() * 20) + 5
        };
        setTimeout(() => {
  ...
  if (stage === "selection") {
    return (
      <div style={{ minHeight: "100vh", background: "#f5f5f7", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: "#1d1d1f", paddingTop: 120, paddingBottom: 80, overflowX: "hidden" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 60 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 14px", background: "rgba(0,113,227,0.08)", borderRadius: 20, color: "#0071e3", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 20 }}>
              <Terminal size={12} /> Evaluation Engine
            </div>
            <h1 style={{ fontSize: "clamp(36px, 5vw, 56px)", fontWeight: 700, letterSpacing: "-0.04em", lineHeight: 1.05, marginBottom: 18 }}>Select an infrastructure track.</h1>
            <p style={{ fontSize: 19, color: "#86868b", maxWidth: 600, margin: "0 auto", lineHeight: 1.5 }}>Begin a deterministic evaluation to generate your verified capability profile.</p>
          </div>
          
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
            {[
              "Frontend Systems Engineering", "Backend Infrastructure", "AI / ML Engineering", "Cloud & DevOps", 
              "Distributed Systems", "Database Engineering", "Cybersecurity Infrastructure", "System Design", 
              "DSA Intelligence", "Product Engineering"
            ].map(t => (
              <div 
                key={t} 
                onClick={() => handleStart(t)}
                style={{
                  background: "#ffffff", borderRadius: 24, padding: 28, border: "1px solid rgba(0,0,0,0.05)",
                  cursor: "pointer", transition: "all 0.3s cubic-bezier(0.25, 1, 0.5, 1)", boxShadow: "0 4px 20px rgba(0,0,0,0.03)"
                }}
              ...
              >
                <div style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(0,113,227,0.08)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
                  <Database size={20} color="#0071e3" />
                </div>
                <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, letterSpacing: "-0.02em" }}>{t}</h3>
                <p style={{ fontSize: 14, color: "#86868b", lineHeight: 1.5 }}>Enterprise-grade assessment of core {t.split(" ")[0].toLowerCase()} competencies.</p>
              </div>
            ))}
          </div>
        </div>
  ...
  if (stage === "report" && reportData) {
    return (
      <div id="report-content" style={{ minHeight: "100vh", background: "#f5f5f7", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: "#1d1d1f", paddingTop: 120, paddingBottom: 80 }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 50 }}>
            <div style={{ width: 64, height: 64, borderRadius: 20, background: "#34c759", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", boxShadow: "0 12px 24px rgba(52,199,89,0.2)" }}>
              <ShieldCheck size={32} color="#ffffff" />
            </div>
            <h1 style={{ fontSize: 48, fontWeight: 700, letterSpacing: "-0.04em", marginBottom: 12 }}>Capability Profile</h1>
            <p style={{ fontSize: 18, color: "#86868b" }}>Evaluation complete for {track}.</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 24, alignItems: "start" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              {/* Vectors Card */}
              <div style={{ background: "#ffffff", borderRadius: 24, padding: 32, border: "1px solid rgba(0,0,0,0.05)", boxShadow: "0 4px 24px rgba(0,0,0,0.02)" }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 24, display: "flex", alignItems: "center", gap: 8 }}>
                  <Activity size={18} color="#0071e3" /> Engineering Vectors
                </h3>
                  ...
                  {Object.entries(reportData.vectors).map(([k, v]) => (
                    <div key={k}>
                      ...
                      <div style={{ width: "100%", height: 8, background: "rgba(0,0,0,0.05)", borderRadius: 4, overflow: "hidden" }}>
                        <div style={{ width: `${v}%`, height: "100%", background: v > 80 ? "#34c759" : v > 60 ? "#0071e3" : "#ff9500", borderRadius: 4 }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ background: "#ffffff", borderRadius: 24, padding: 32, border: "1px solid rgba(0,0,0,0.05)", boxShadow: "0 4px 24px rgba(0,0,0,0.02)" }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20, display: "flex", alignItems: "center", gap: 8 }}>
                  <Network size={18} color="#0071e3" /> Recommended Roadmap
                </h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {reportData.roadmap.map((node, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: 16, background: "#f5f5f7", borderRadius: 16, fontSize: 15, fontWeight: 500 }}>
                      <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center", color: "#86868b", fontSize: 12, fontWeight: 700 }}>{i+1}</div>
                      {node}
                    </div>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              {/* Benchmark Card */}
              <div style={{ background: "#ffffff", borderRadius: 24, padding: 32, border: "1px solid rgba(0,0,0,0.05)", boxShadow: "0 4px 24px rgba(0,0,0,0.02)", textAlign: "center" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#86868b", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 12 }}>Global Alignment</div>
                <div style={{ fontSize: 48, fontWeight: 700, letterSpacing: "-0.04em", color: "#1d1d1f" }}>Top {reportData.benchmark}%</div>
                <div style={{ fontSize: 14, color: "#86868b", marginTop: 8 }}>among evaluated enterprise profiles.</div>
              </div>

              {/* Recruiter Card */}
              <div style={{ background: "#ffffff", borderRadius: 24, padding: 32, border: "1px solid rgba(0,0,0,0.05)", boxShadow: "0 4px 24px rgba(0,0,0,0.02)" }}>
                <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                  <Layers size={16} color="#0071e3" /> Confidence Engine
                </h3>
                <div style={{ padding: 16, background: "rgba(52,199,89,0.1)", borderRadius: 16, color: "#1d1d1f", fontSize: 15, fontWeight: 600 }}>
                  <span style={{ color: "#34c759", marginRight: 8 }}>{reportData.recruiter_signal}</span> identified.
                </div>
              </div>
              
              <button 
                onClick={handleExportPDF}
                style={{ padding: 16, borderRadius: 100, background: "#0071e3", color: "white", fontWeight: 600, fontSize: 16, border: "none", cursor: "pointer", transition: "background 0.2s" }}
                onMouseOver={e => e.currentTarget.style.background = "#005bbf"}
              ...
              <button 
                ...
                style={{ padding: 16, borderRadius: 100, background: "#ffffff", color: "#1d1d1f", fontWeight: 600, fontSize: 16, border: "1px solid rgba(0,0,0,0.1)", cursor: "pointer", transition: "background 0.2s" }}
  ...
  // --- STAGES: ORCHESTRATOR / ANALYSIS (LOADING) ---
  if (stage === "orchestrator" || stage === "analysis") {
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(255,255,255,0.88)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
        <div style={{ background: "#ffffff", borderRadius: 28, padding: 52, border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 32px 80px rgba(0,0,0,0.08)", textAlign: "center", maxWidth: 400, width: "90%" }}>
          <div style={{ position: "relative", width: 72, height: 72, margin: "0 auto 28px" }}>
            <div className="spinner-border" style={{ position: "absolute", inset: 0, borderRadius: "50%", border: "3px solid rgba(0,113,227,0.12)", borderTopColor: "#0071e3", animation: "spin 1.4s linear infinite" }} />
            <div style={{ position: "absolute", inset: 8, borderRadius: "50%", background: "rgba(0,113,227,0.07)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Zap size={22} color="#0071e3" />
            </div>
          </div>
          <style>{`@keyframes spin { 100% { transform: rotate(360deg); } }`}</style>
          <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", color: "#1d1d1f", marginBottom: 10 }}>
            {stage === "orchestrator" ? "Initializing Protocol..." : "Analyzing Vectors..."}
          </h2>
          <p style={{ fontSize: 15, color: "#86868b", lineHeight: 1.6 }}>
            {stage === "orchestrator" ? "Synchronizing infrastructure..." : "Evaluating systems reasoning capability..."}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f7", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: "#1d1d1f", paddingTop: 120, paddingBottom: 80, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: 800, padding: "0 24px" }}>
        
        {/* Progress Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#86868b", textTransform: "uppercase", letterSpacing: "0.05em" }}>{track}</div>
          <div style={{ fontSize: 15, fontWeight: 600, color: "#1d1d1f" }}>{currentIndex + 1} of {questions.length}</div>
        </div>
        
        {/* Progress Bar */}
        <div style={{ width: "100%", height: 8, background: "rgba(0,0,0,0.06)", borderRadius: 4, overflow: "hidden", marginBottom: 40 }}>
          <div style={{ width: `${progressPercent}%`, height: "100%", background: "#0071e3", transition: "width 0.4s ease" }} />
        </div>
        
        {/* Question Card */}
        <div style={{ background: "#ffffff", borderRadius: 32, padding: "48px 40px", border: "1px solid rgba(0,0,0,0.05)", boxShadow: "0 12px 48px rgba(0,0,0,0.04)" }}>
          <h2 style={{ fontSize: "clamp(24px, 4vw, 32px)", fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1.3, marginBottom: 40, color: "#1d1d1f" }}>
            ...
            {questions[currentIndex].options.map((opt, i) => {
              ...
              return (
                <div 
                  ...
                  style={{
                    padding: 20, borderRadius: 20, border: `2px solid ${isSelected ? "#0071e3" : "rgba(0,0,0,0.06)"}`,
                    background: isSelected ? "#0071e3" : "#ffffff", color: isSelected ? "#ffffff" : "#1d1d1f",
                    cursor: "pointer", transition: "all 0.2s ease", display: "flex", alignItems: "center", gap: 16,
                    fontWeight: 600, fontSize: 17
                  }}
                  onMouseOver={e => { if (!isSelected) e.currentTarget.style.borderColor = "rgba(0,0,0,0.15)"; }}
                  onMouseOut={e => { if (!isSelected) e.currentTarget.style.borderColor = "rgba(0,0,0,0.06)"; }}
                >
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: isSelected ? "rgba(255,255,255,0.2)" : "#f5f5f7", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 40 }}>
            <button 
              ...
              style={{
                padding: "16px 32px", borderRadius: 100, border: "none", fontWeight: 600, fontSize: 16,
                background: selectedOption ? "#0071e3" : "rgba(0,0,0,0.05)",
                color: selectedOption ? "#ffffff" : "#9ca3af",
                cursor: selectedOption ? "pointer" : "not-allowed",
                transition: "all 0.2s ease"
              }}
              onMouseOver={e => { if (selectedOption) e.currentTarget.style.background = "#005bbf"; }}
              onMouseOut={e => { if (selectedOption) e.currentTarget.style.background = "#0071e3"; }}
            >
              Submit Answer
            </button>
          </div>
